...
def get_audio_numpy(file_name, sub=False):
    # 入力:音声ファイルのファイル名
...
def make_dataset(sub=False):
    ...
    for label, hiragana in enumerate(hiraganas):
        for i in range(4):
            file_name = "easily_splittable_hiragana_data/" + hiragana + str(i + 1) + ".m4a"
            sound_arr, frame_rate = get_audio_numpy(file_name, sub=sub)
            if frame_rate == 4410 or frame_rate == 44100:
                print(hiragana, i)
            else:
                sound_arrs = audio_split(sound_arr, frame_rate)
